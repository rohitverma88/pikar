...
selectService: string[];
  ...
  ngOnInit() {
    this.sub = this.route
      .queryParams
      .subscribe(params => {
        this.filterObj.inventoryType = 'PRODUCT';
  ...
  // Method: is used to Navigate user to a desired filter Page
   navigateUser() {
    ...
    const tempLink = '/app/ssp-panel/ssp-inventory/products?' + tempUrlQuery;
  ...
  exportFile() {
    ...
    if (isConfirmed === true) {
      const tempUrlQuery = ProductUtilities.generateQueryString(this.filterObj);
      this.exportFileEntity.getExportData('ssp-pannel-inventory-product', tempUrlQuery);
    }
  }
  ...
  getProductListingData(){
    ...
    this.inventoryService.getInventoryList(productQueryStr).subscribe(
      response => {
  ...
  viewProduct(data) {
    if (!!data.productId && data.productId !== '') {
      ...
      this.router.navigate(['/app/ssp-panel/ssp-inventory/products/view-products'], { queryParams: { productId: data.productId} });
    } else {
      this.toastrService.showError('Error', 'Something went wrong please try again later');
    }
  }
  editProduct(data) {
    if (!!data.productId && data.productId !== '') {
      // tslint:disable-next-line: max-line-length
      this.router.navigate(['/app/ssp-panel/ssp-inventory/products/add-edit-products'], { queryParams: { productId: data.productId, type: 'edit'}});
    } else {
      this.toastrService.showError('Error', 'Something went wrong please try again later');
    }
  }
...
gettypeDropdown() {
  const typeQueryStr = '?name=["PRODUCTS"]';
  this.inventoryService.getENUMData(typeQueryStr).subscribe(
    response => {
      if (!!response && !!response.success && response.success === true && response.response.data.length !== 0 ) {
        ...
        this.toastrService.showError('Error', 'No Category Found!');
      }
    },
    err => {
...
submitFilter(data) {
  if(!(!!data.category || !!data.date || !!data.instock)) {
...
// Remove All Filters
removeFilters() {
  this.router.navigate(['/app/ssp-panel/ssp-inventory/products'], { queryParams: { pageSize: 50, currentPage: 1}});
  this.getProductListingData();
}
// End of Code
}
