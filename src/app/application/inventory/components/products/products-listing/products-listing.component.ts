  ...
  ngOnInit() {
    this.sub = this.route
      ...
      .subscribe(params => {
        ...
        this.filterObj.subcategory = params.subcategory || '';
  ...
  this.inventoryService.gettypeDropdown(typeQueryStr).subscribe(
    response => {
      if (!!response && !!response.success && response.success === true && response.response.data.length !== 0 ) {
        ...
        this.categoryDropdownData = [];
        this.toastrService.showError('Error', 'No Sub Category Found!');
      }
    },
    err => {
        this.toastrService.showError('Network Error', 'Please try again later');
...
submitFilter(data) {
  if(!(!!data.category || !!data.date || !!data.instock || !!data.stock_units || !!data.vendorId)) {
