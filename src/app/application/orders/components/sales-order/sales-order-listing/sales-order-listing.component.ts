...
selectService: string[];
  ...
  // Method: is used to Navigate user to a desired filter Page
   navigateUser() {
    ...
    const tempLink = '/app/orders/sales-order?' + tempUrlQuery;
  ...
  exportFile() {
    ...
    if (isConfirmed === true) {
      const tempUrlQuery = ProductUtilities.generateQueryString(this.filterObj);
      this.exportFileEntity.getExportData('orders/sales-order/', tempUrlQuery);
    }
  }
  ...
  viewOrder(data) {
    if (!!data.orderId && data.orderId !== '') {
      ...
      this.router.navigate(['/app/orders/sales-order/view-sales-order'], { queryParams: { orderId: data.orderId} });
    } else {
      this.toastrService.showError('Error', 'Something went wrong please try again later');
    }
  }
  ...
  completeOrder(data) {
    if (!!data.orderId && data.orderId !== '') {
      // tslint:disable-next-line: max-line-length
      this.router.navigate(['/app/orders/sales-order/complete-sales-order'], { queryParams: { orderId: data.orderId} });
    } else {
      this.toastrService.showError('Error', 'Something went wrong please try again later');
    }
  }
  editOrder(data) {
    if (!!data.orderId && data.orderId !== '') {
      // tslint:disable-next-line: max-line-length
      this.router.navigate(['/app/vendor/add-edit-orders'], { queryParams: { orderId: data.orderId, type: 'edit'}});
    } else {
      this.toastrService.showError('Error', 'Something went wrong please try again later');
    }
  }
...
removeFilters() {
  this.router.navigate(['/app/orders/sales-order/'], { queryParams: { pageSize: 50, currentPage: 1}});
  this.getOrderListingData();
}
// End of Method
}
